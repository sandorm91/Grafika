...
void bounce_ball(Pong* pong)
{
    if (pong->ball.x - pong->ball.radius < 50) {
        ...
        {
            //printf("ball: %f, left pad top: %f, left pad bottom: %f \n", pong->ball.y, pong->left_pad.y, pong->left_pad.y-pong->left_pad.height);
    ...
    if (pong->ball.x + pong->ball.radius > pong->width - 50) {
        ...
        {
            printf("ball: %f, right pad top: %f, right pad bottom: %f \n", pong->ball.y, pong->right_pad.y, pong->right_pad.y+pong->right_pad.height);
            pong->ball.x=pong->width/2;
            pong->ball.y=pong->height/2;
        }
        pong->ball.speed_x *= -1;
